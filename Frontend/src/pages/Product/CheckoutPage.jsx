...
//  React Icons
import { 
  FiMapPin, FiCreditCard, FiCheckCircle, FiUser, FiPhone, FiMail, FiHome, FiPackage, FiDollarSign, FiCornerRightDown
} from "react-icons/fi";
...
export default function CheckoutPage() {
  ...
  const totalPrice = checkoutItems.reduce(
    (sum, item) => sum + (item.productId?.price || item.product?.price || 0) * item.quantity,
    0
  );
 ...
 const validateAddress = () => {
  ...
  const createOrderOnServer = async (method) => {
    try {
      ...
      const { data } = await axios.post(`${API}/api/user/order`, payload, { withCredentials: true });
      return data.order;
    } catch (err) {
      console.error(err);
  ...
  const StepCircle = ({ step, label, icon: Icon }) => {
    ...
    return (
      <div className="flex flex-col items-center relative">
        ...
        >
          {isCompleted ? <FiCheckCircle className="w-5 h-5"/> : <Icon className="w-5 h-5"/>}
        </div>
        <span
  ...
  const renderAddressInput = ({ name, placeholder, type = "text", colSpan = "" }) => (
    <div key={name} className={colSpan}>
      <label className="block text-gray-700 font-medium mb-1 capitalize text-sm">{placeholder.replace('Enter ', '')}</label>
      <input
        type={type}
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return (
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2"><FiMapPin className="text-blue-500"/> Shipping Address</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {renderAddressInput({ name: "fullName", placeholder: "Enter Full Name", colSpan: "sm:col-span-2" })}
              {renderAddressInput({ name: "phone", placeholder: "Enter Phone No.", type: "tel" })}
              {renderAddressInput({ name: "email", placeholder: "Enter Email", type: "email" })}
              {renderAddressInput({ name: "address", placeholder: "Enter Full Address", colSpan: "sm:col-span-2" })}
              {renderAddressInput({ name: "city", placeholder: "Enter City" })}
              {renderAddressInput({ name: "state", placeholder: "Enter State" })}
              {renderAddressInput({ name: "pincode", placeholder: "Enter Pin Code" })}
            </div>
            <button
              onClick={handleNext}
      ...
      case 2:
        return (
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2"><FiCreditCard className="text-blue-500"/> Payment Method</h2>
              ...
              {["Cash on Delivery", "Online"].map((method) => (
                ...
                >
                  ...
                  <span className="ml-4 text-gray-900 font-semibold text-base flex-1">
                    {method === "Online" ? "Pay Online (Card/UPI/Net Banking)" : "Cash on Delivery (COD)"}
                  </span>
                  {method === "Online" && <FiCreditCard className="w-6 h-6 text-green-500" />}
                  {method === "Cash on Delivery" && <BsCashCoin  className="w-6 h-6 text-indigo-500" />}
                </label>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row items-center justify-between mt-8 gap-4">
              ...
              {paymentMethod === "Cash on Delivery" ? (
                <button
                  ...
                  className={`w-full sm:w-auto bg-gradient-to-r from-blue-500 to-indigo-500 text-white py-3 px-8 rounded-xl shadow-lg hover:from-blue-600 hover:to-indigo-600 transition-all font-semibold text-lg ${isProcessing ? "opacity-60 cursor-not-allowed" : ""}`}
                  disabled={isProcessing}
                >
                ...
                <button
                  ...
                  className={`w-full sm:w-auto bg-gradient-to-r from-green-500 to-teal-500 text-white py-3 px-8 rounded-xl shadow-lg hover:from-green-600 hover:to-teal-600 transition-all font-semibold text-lg ${isProcessing ? "opacity-60 cursor-not-allowed" : ""}`}
                  disabled={isProcessing}
                >
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="container mx-auto px-4 py-10 lg:py-14 max-w-7xl">
        <h1 className="text-3xl font-extrabold text-gray-900 mb-8 border-b pb-4">Secure Checkout</h1>
          ...
          <div className="lg:col-span-7 order-last lg:order-first">
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl">
              <div className="flex justify-between items-center mb-10 w-full max-w-md mx-auto">
                ...
                <div className="flex-1 h-1 mt-6 mx-2 rounded-full transition-colors" style={{backgroundColor: currentStep > 1 ? '#34D399' : '#E5E7EB'}} />
                <StepCircle step={2} label="Payment" icon={FiCreditCard} />
                <div className="flex-1 h-1 mt-6 mx-2 rounded-full transition-colors" style={{backgroundColor: currentStep > 2 ? '#34D399' : '#E5E7EB'}} />
                <StepCircle step={3} label="Confirmation" icon={FiCheckCircle} />
              </div>
              {renderStepContent()}
            </div>
          </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl lg:sticky lg:top-10 border-2 border-gray-200">
              ...
              <div className="space-y-4 max-h-72 overflow-y-auto pr-3 border-b pb-4">
                {checkoutItems.map((item, idx) => {
                  const itemPrice = (item.productId?.price || item.product?.price || 0) * item.quantity;
                  return (
                    <div key={idx} className="flex justify-between items-center p-2 rounded-lg bg-gray-50 hover:bg-gray-100 transition">
                      <div className="flex items-center gap-4">
                        <img
                          src={item.productId?.imageUrl || item.product?.imageUrl || 'https://via.placeholder.com/64'}
                      ...
                      <span className="font-semibold text-gray-800 text-sm shrink-0">₹{itemPrice.toLocaleString('en-IN')}</span>
                    </div>
                  );
                })}
              </div>
              <div className="pt-6 mt-4 space-y-3 text-sm">
                ...
                <div className="flex justify-between text-gray-600">
                  ...
                  <span className="font-medium text-green-600">{shippingCharge === 0 ? 'FREE' : `+ ₹${shippingCharge}`}</span>
                </div>
                ...
                {currentStep === 2 && (
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg text-sm">
                    <p className="font-semibold text-blue-800 flex items-center"><FiCornerRightDown className="mr-1"/> Delivering to:</p>
                    <p className="text-xs text-blue-600 pl-4"><p>Name: {address.fullName}</p> <p>Address: {address.address}, {address.city}, {address.pincode}</p>
